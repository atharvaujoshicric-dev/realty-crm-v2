...
async function navigate(pageId) {
  ...
  const reloadPages = { 'p-dash': renderDash, 'p-bookings': renderBookings,
    ...
    'p-settings': renderSettings, 'p-sa-projects': renderSAProjects,
    'p-sa-users': renderSAUsers, 'p-sa-import': renderImportPage };
  if (pageId !== 'p-sa-projects' && pageId !== 'p-sa-users' && pageId !== 'p-sa-import' && S.curProj) await loadProjData();
  reloadPages[pageId]?.();
}
...
async function deleteProject(pid, pname) {
  if (!confirm(`Delete project "${pname}"?\n\nThis will permanently delete ALL bookings, cheques, and records for this project.\n\nType the project name to confirm.`)) return;
  const typed = prompt(`Type "${pname}" to confirm deletion:`);
  if (typed !== pname) { toast('Project name did not match — deletion cancelled', 'err'); return; }
  ...
  if (error) { toast('Error deleting project: ' + error.message, 'err'); return; }

  // If we were viewing this project, reset
  if (S.curProj?.id === pid) {
    S.curProj = null;
    el('projSwitcher').style.display = 'none';
  }
...
async function saveProj() {
  ...
  // Now create users via signUp (works without Admin API)
  const aname = v('pm-aname').trim(), amail = v('pm-amail').trim(), apass = v('pm-apass').trim();
  if (aname && amail && apass) {
    const adminId = await createUserViaSignup(amail, apass, aname, 'admin', proj.id);
    if (!adminId) { setBtn('pm-savebtn',false); return; }
  }

  const sname = v('pm-sname').trim(), smail = v('pm-smail').trim(), spass = v('pm-spass').trim();
  if (sname && smail && spass) {
    await createUserViaSignup(smail, spass, sname, 'sales', proj.id);
  }

  setBtn('pm-savebtn', false);
  toast('Project created! Users have been sent confirmation emails.');
  closeM('projM');
  await renderSAProjects();
}

async function createUserViaSignup(email, password, name, role, projectId) {
  // Try signUp
  ...
  // Handle "already registered" — user may already exist in auth but not have a profile
  if (error && !error.message.includes('already registered')) {
    // Show helpful instructions instead of cryptic error
    showUserCreationInstructions(email, name, role, projectId);
    return null;
  }
  ...
  // Insert profile
  const { error: profErr } = await sb.from('profiles').upsert({ id: uid, full_name: name, role });
  if (profErr) {
    showUserCreationInstructions(email, name, role, projectId);
    return null;
  }
  ...
  // Assign to project
  if (projectId) {
    await sb.rpc('assign_to_project', { p_user_id: uid, p_project_id: projectId, p_role: role });
  }

  return uid;
}

function showUserCreationInstructions(email, name, role, projectId) {
  // Get project name for the instructions
  const proj = S.projects.find(p => p.id === projectId) || S.curProj;
  const projName = proj?.name || 'your project';
  const projId = projectId || proj?.id || 'PROJECT_ID';

  const sql = `-- Step 1: Run after creating user in Supabase Dashboard
...
SELECT '${projId}', id, '${role}'
FROM auth.users WHERE email = '${email}'
ON CONFLICT DO NOTHING;`;

  el('manual-create-sql').textContent = sql;
  el('manual-create-email').textContent = email;
  el('manual-create-pass').textContent = '(the password you entered)';
  openM('manualUserM');
}
...
async function saveUser() {
  ...
  const uid = await createUserViaSignup(email, pass, name, role, projId||null);
  
  setBtn('um-savebtn',false);
  if (!uid) return;
  closeM('userM');
  if (S.profile?.role==='superadmin') await renderSAUsers(); else renderSettings();
  toast('User created! They can now sign in.');
}

async function removeUser(uid) {
  if (!confirm('Remove this user from the CRM? They will lose all access.')) return;
  ...
  // Try to delete from auth via RPC
  await sb.rpc('delete_crm_user', { p_user_id: uid }).catch(() => {});
  
  toast('User removed!');
  if (S.profile?.role==='superadmin') await renderSAUsers(); else renderSettings();
}

async function removeFromProject(uid, pid) {
...
function copySQL() {
  const sql = el('manual-create-sql')?.textContent || '';
